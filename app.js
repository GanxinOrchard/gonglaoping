...
const CONFIG = {
  BRAND_TAG: "柑心果園",
  GAS_ENDPOINT: "https://script.google.com/macros/s/AKfycbzT7yzMZXqjpJq_AvbcCKUrZaH3-N74YoRdsj3c4V2gfhD5Rbdnf3oucVvnextsrbhu/exec", // 如有變更，改這裡
  SHIPPING: 160,
...
(function renderPraise(){
  const msgs = ["買過都說讚","團購主回購中","冰過更清甜","手剝不黏手","回購率超高"];
...
function renderSpec(kind){
  ...
  // 動態尺寸
  document.getElementById('size-'+kind.toLowerCase()).textContent = SIZE_CM[size] || '—';
}
...
/**************
 * 果實近拍 1:1 輪播（手機點點切換）＋ 點圖放大
 **************/
(function fruit(){
  const track = document.getElementById('fruitTrack');
  const dots = document.getElementById('fruitDots');
  const imgs = [...track.querySelectorAll('img')];
  imgs.forEach(img=>{ img.src = img.getAttribute('data-img'); img.addEventListener('click', ()=>openLightbox(img.src)); });
  if(window.matchMedia('(max-width: 900px)').matches){
    let idx=0;
    imgs.forEach((_,i)=>dots.insertAdjacentHTML('beforeend',`<button ${i===0?'class="active"':''} aria-label="第 ${i+1} 張"></button>`));
    const btns=[...dots.querySelectorAll('button')];
    function go(i){ idx=i; btns.forEach((b,j)=>b.classList.toggle('active',j===idx)); track.style.transform=`translateX(${-idx*100}%)`; track.style.display='grid'; track.style.gridTemplateColumns=`repeat(${imgs.length},100%)`; track.style.transition='transform .35s ease'; }
...
// Lightbox
function openLightbox(src){ const lb=document.getElementById('lightbox'); document.getElementById('lightboxImg').src=src; lb.classList.add('show'); }
    ...
    if(payMethod==='BANK'){
      resBox.innerHTML = `✅ 訂單已建立（編號：<b>${orderNo}</b>）。<br>
        ...
        <div class="card" style="padding:10px;margin-top:8px">
          <div><b>連線銀行(824)</b></div>
          <div>戶名：<b>張鈞泓</b></div>
          <div>帳號：<b>11101-37823-13</b></div>
        </div>`;
    }
    if(payMethod==='CASH_PICKUP'){
      resBox.innerHTML = `✅ 訂單已建立（編號：<b>${orderNo}</b>）。<br>
        您選擇 <b>自取（現金）</b>，到場請報訂單編號即可。`;
    }
